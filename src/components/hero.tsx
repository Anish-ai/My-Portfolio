...
const Particle = ({ index, mousePosition }: { index: number; mousePosition: { x: number; y: number } }) => {
  ...
  const initialX = Math.random() * 100; // Initial X position (%)
  const initialY = Math.random() * 100; // Initial Y position (%)
  const intensity = Math.random() * 0.2 + 0.05; // How strongly particle reacts to mouse (0.05-0.25)
  ...
  return (
    <motion.div
      ...
      animate={{
        x: [
          0 + x * 0.2,
          Math.random() * 200 - 100 + x * 0.5,
          Math.random() * 200 - 100 + x * 0.3,
          Math.random() * 200 - 100 + x * 0.4,
          0 + x * 0.2,
        ],
        y: [
          0 + y * 0.2,
          Math.random() * 200 - 100 + y * 0.5,
          Math.random() * 200 - 100 + y * 0.3,
          Math.random() * 200 - 100 + y * 0.4,
          0 + y * 0.2,
        ],
...
export default function Hero() {
  ...
  const handleEmailClick = () => {
    window.location.href = `mailto:aniskum59431@gmail.com?subject=Let's Work Together`
  }
  ...
  const particles = Array.from({ length: 30 }, (_, index) => (
    <Particle key={index} index={index} mousePosition={normalizedMousePosition} />
  ));
    ...
    >
      ...
      <div className="container mx-auto px-4 z-10">
          ...
          >
            <div ref={profileImageRef} className="relative w-64 h-64 md:w-80 md:h-80 perspective-1000">
              ...
              <motion.div
                ...
                style={{ 
                  background: "radial-gradient(circle, rgba(139, 92, 246, 0.6) 0%, rgba(217, 70, 239, 0.3) 50%, transparent 70%)",
                  filter: "blur(10px)"
                }}
                animate={{
                  scale: [1, 1.2, 1],
        ...
        >
          ...
          >
            <span className="text-sm text-gray-400 mb-2 group-hover:text-violet-400 transition-colors">Scroll Down</span>
            <motion.div
              animate={{
                y: [0, 5, 0]
              }}
